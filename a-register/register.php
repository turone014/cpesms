...
if ($stmt = $con->prepare('SELECT id, password FROM reg_chair WHERE username = ?')) {
	...
	if ($stmt->num_rows > 0) {
		...
		$outputs = '<h3>Username exist, please recover your account by using forgot password</h3>';
	} else {
		// Username doesnt exists, insert new account
        if ($stmt = $con->prepare('INSERT INTO reg_chair (username, firstname, lastname, email, password, activation_code ) VALUES (?, ?, ?, ?, ?, ?)')) {
...
$stmt->bind_param('ssssss', $usernamess, $firstnames, $lastnames, $_POST['email'], $password, $uniqid);

	$stmt->execute();
	$from    = 'noreply@yourdomain.com';
$subject = 'Account Activation Required';
$headers = 'From: ' . $from . "\r\n" . 'Reply-To: ' . $from . "\r\n" . 'X-Mailer: PHP/' . phpversion() . "\r\n" . 'MIME-Version: 1.0' . "\r\n" . 'Content-Type: text/html; charset=UTF-8' . "\r\n";
$activate_link = 'http://localhost/cpe-sms/a-register/activate.php?email=' . $_POST['email'] . '&code=' . $uniqid;
...
"> 
   ...
   <a href="../a-forgot" class="forgot-password-link">Forgot password?</a>
